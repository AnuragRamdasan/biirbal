...
customAdapter.getUserByEmail = async (email: string) => {
  try {
    ...
    // FIX Bug 1: Only suppress P2002 (unique constraint) errors which indicate
    // a legitimate cross-provider account link attempt. Rethrow all other errors
    // (DB timeouts, connection failures) so they surface properly instead of
    // silently returning null and creating a duplicate user.
    if (error?.code === 'P2002') {
        ...
        try {
          ...
          if (isNewTeam) {
            try {
              ...
              // Slack team installation tracking handled client-side
            } catch (error) {
          ...
          if (userId && userAccessToken) {
            ...
            if (!existingMembership) {
              ...
              if (!canAdd.allowed) {
                ...
                // FIX Bug 2a: Return error redirect immediately instead of setting flag and falling through.
                // Previously userSeatAllowed was set to false but signIn() always returned true anyway.
      ...
      if (session.user && user) {
        ...
        if (dbUser) {
          ...
          if (defaultTeam) {
            ...
            // FIX Bug 2b: Users with no active memberships must be flagged explicitly.
            // Without this, session.user.teamId remains undefined, causing silent failures
            // downstream in API routes that assume teamId is always set.
