...
export async function processLink(params: ProcessLinkParams, updateProgress?: (progress: number) => Promise<void>): Promise<void> {
  ...
  try {
    ...
    // FIX Bug 3: Mark the processedLink record as FAILED before rethrowing.
    // Previously the DB record stayed in PROCESSING state indefinitely when errors occurred,
    // making it impossible to distinguish stuck jobs from genuinely failed ones.
