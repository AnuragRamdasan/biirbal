...
export async function updateSubscriptionFromStripe(
  ...
  planId: string,
  status: string,
  // Bug 5 fix: The original function always hardcoded currentPeriodEnd = now + 30 days,
  // ignoring the real billing period from Stripe. Annual subscribers saw their subscription
  // expire in 30 days immediately after payment. Now callers can pass the real Stripe
  // period end date; the 30-day value is kept as a safe fallback for backward compatibility.
