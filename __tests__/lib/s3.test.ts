...
describe('s3', () => {
  ...
  it('should upload buffer to S3', async () => {
    ...
    expect(result).toBe('https://test-bucket.s3.us-east-1.amazonaws.com/test.mp3');
  });

  it('should handle upload errors', async () => {
    mockS3.promise.mockRejectedValue(new Error('Upload failed'));

    await expect(uploadToS3(Buffer.from('test'), 'test.mp3'))
      .rejects
      .toThrow('Failed to upload audio to S3');
  });
}); 