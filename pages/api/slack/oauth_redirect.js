...
export default async function handler(req, res) {
  ...
  try {
    ...
    res.setHeader('Set-Cookie', [
      `workspaceId=${result.team.id}; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=${60 * 60 * 24 * 7}`, // 7 days
      `workspaceName=${encodeURIComponent(result.team.name)}; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=${60 * 60 * 24 * 7}`
    ]);
